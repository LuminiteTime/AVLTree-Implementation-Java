...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            avlTree.insert(value, i + 1);
        }
        System.out.println(n);
        printResult(avlTree.getRoot());
        System.out.println(avlTree.getRoot().getIndex());
    }
    ...
    private static void printOneNode(Node<Integer> node) {
        if (node != null) {
            ...
            System.out.println(node.getValue() + " " + (node.getLeft() == null ? -1 : node.getLeft().getIndex()) + " " +
                    (node.getRight() == null ? -1 : node.getRight().getIndex()));
